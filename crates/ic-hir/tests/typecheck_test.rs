...
fn test_string_assigned_to_int() {
    ...
    assert!(!result.errors.is_empty(), "Expected type error for string assigned to int, but got no errors");
}
...
fn test_int_overflow() {
    ...
    assert!(!result.errors.is_empty(), "Expected overflow error for int8 = 256, but got no errors");
}
...
#[test]
#[ignore] // Setting the size of an enum would require handling annotations which we don't do just yet
fn test_enum_value_overflow() {
    ...
    // Should have an overflow error
    assert!(!result.errors.is_empty(), "Expected overflow error for enum value 200 in int8, but got no errors");
}
...
fn test_union_case_type_mismatch() {
    ...
    // Should have a type error
    assert!(!result.errors.is_empty(), "Expected type error for string case label with int32 discriminator, but got no errors");
}